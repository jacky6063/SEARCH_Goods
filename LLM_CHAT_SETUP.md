...
- 透過後端診斷端點檢查 API key 與旗標：
```bash
curl -s http://localhost:8000/api/debug/llm | python3 -m json.tool
```
重點欄位：
- openai_api_key_status：not_set / placeholder / valid_key_****
- chat_use_expand / chat_use_intent：聊天 LLM 旗標
- search_use_expand / search_use_intent：搜尋 LLM 旗標
- chat_openai_model / search_openai_model：使用中的模型
...
- [x] 設置有效的 `OPENAI_API_KEY` ✅（`/api/debug/llm` 顯示 valid_key_*）
- [x] 確認 `USE_CHAT_MODE=True`（預設為 True） ✅
- [x] `CHAT_OPENAI_MODEL=gpt-4o-mini` ✅（或在 `/api/debug/llm` 檢查）
- [ ] 依需求設定 `CHAT_USE_LLM_INTENT/EXPAND/RERANK/PROMO`（預設 True/True/False/True）
- [ ] 部署更新到 Render
- [ ] 測試自然語言對話
