...
輕量商品查詢客服系統 MVP 範例ＡAＢC。

快速啟動參考請見 `backend/README.md`。示例 URL 寫法規範請見 `docs/示例URL規範.md` 與 `docs/術語對照快表.md`。
...
## GitHub Actions 自動部署（目前僅 Render）

當程式推送到 `main` 分支時，`.github/workflows/deploy.yml` 會先執行後端測試，通過後觸發 Render 重新部署。請在 GitHub 專案的 Secrets 中設定下列項目：

- `RENDER_SERVICE_ID`：Render 後端服務的 ID。
- `RENDER_API_KEY`：Render API key，需具備部署權限。

注意：原 Netlify 前端站台已停用，相關文件已歸檔於 `docs/archive/2025-11/`。
